         ...
         if not onlyFit:
            for (_cat,yr) in itertools.product(categories,years):
               ...
               if not (os.path.exists(f) and os.path.exists(f0)):
                  ...
                  out.append("MYTMPFILE=\$(mktemp); python susy-sos/sos_plots.py --lep %s --reg %s --bin %s --data --asimov background --doWhat cards --unc --fakes=semidd %s --signal --signalMasses %s --allowRest --infile %s_merged/bare %s > \${MYTMPFILE}; source \${MYTMPFILE}; rm \${MYTMPFILE};"%(lep,reg,bin,opts,pr,odir,yr))
