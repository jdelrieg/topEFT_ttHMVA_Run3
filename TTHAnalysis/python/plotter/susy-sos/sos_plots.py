...
def base(selection):
    ...
    if selection=='2los':
        ...
        if args.doWhat in ["cards"]:
            if args.reg in ["sr","cr_ss"]: plotting+=" 'mass_2(LepGood1_pt, LepGood1_eta, LepGood1_phi, LepGood1_mass, LepGood2_pt, LepGood2_eta, LepGood2_phi, LepGood2_mass)' [1,4,10,20,30,50] "
            if args.reg in ["cr_dy","cr_tt"]: plotting+=" 1 [-0.5,0.5] "
        ...
        if args.doWhat in ["cards"]:
            if args.reg in ["sr"]: plotting+="  minMllSFOS [1,4,10,20,30,50] "
            if args.reg in ["cr_wz"]: plotting+=" 1 [-0.5,0.5] "
    ...
    if '2los_' in torun:
        ...
        if 'cr_ss' in torun: # Only 'med' bin exists
            ...
            if '1F_NoSF' in torun:
                ...
                x = add(x,"--mcc susy-sos/fakerate/%s/%s/ScaleFactors_SemiDD/mcc_SF_cr_ss.txt "%(YEAR,args.lep))
                x = add(x, "-E ^2LNT$ -X ^twoTight$")
            if args.fakes == "semidd": x = add(x, "--mcc susy-sos/fakerate/%s/%s/ScaleFactors_SemiDD/mcc_SF_cr_ss.txt"%(YEAR,args.lep))
