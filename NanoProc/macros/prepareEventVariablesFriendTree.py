...
if _getarg(("-t","--tree"),"NanoAOD") == "NanoAOD":
    ...
    if "--tra2" in sys.argv:
        ...
        from CMGTools.NanoProc.treeReAnalyzer2 import Module, EventLoop, Booker, PyTree
    else:
        print("Will use the CMGTools version of treeReAnalyzer")
        from CMGTools.NanoProc.treeReAnalyzer import Module, EventLoop, Booker, PyTree
        ...
        for sample, chunks in chunks_with_subs.items():
            for chunk in chunks:
                if done_subchunks[(sample,chunk)] == allsubs:
                    print("%s chunk %s has all the fine splits -> doing the hadd " % (sample, chunk), end=' ')
