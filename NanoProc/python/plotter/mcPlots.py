...
from CMGTools.NanoProc.plotter.mcAnalysis import *
import CMGTools.NanoProc.plotter.CMS_lumi as CMS_lumi
...
if "/bin2Dto1Dlib_cc.so" not in ROOT.gSystem.GetLibraries():
    ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/NanoProc/python/plotter/bin2Dto1Dlib.cc+" % os.environ['CMSSW_BASE']);
if "/fakeRate_cc.so" not in ROOT.gSystem.GetLibraries(): 
    ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/NanoProc/python/plotter/fakeRate.cc+" % os.environ['CMSSW_BASE']);
...
def doRatioHists(pspec,pmap,total,maxRange,fixRange=False,fitRatio=None,errorsOnRef=True,ratioNums="signal",ratioDen="background",ylabel="Data/pred.",yndiv=505,doWide=False,showStatTotLegend=False,textSize=0.035):
    ...
    if fitRatio != None and len(ratios) == 1:
        from CMGTools.NanoProc.tools.plotDecorations import fitTGraph
            ...
            for pspec in pspecs:
                ...
                if self._options.getHistosFromFile:
                    ...
                    if options.noStackSig:
                        ...
                        for i in range(total_for_err.GetNbinsX()):
