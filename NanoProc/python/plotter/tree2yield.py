...
from CMGTools.NanoProc.plotter.cutsFile import CutsFile
from CMGTools.NanoProc.plotter.mcCorrections import *
from CMGTools.NanoProc.plotter.fakeRate import *
from CMGTools.NanoProc.plotter.uncertaintyFile import *
from CMGTools.NanoProc.plotter.histoWithNuisances import HistoWithNuisances, cropNegativeBins

if "/functions_cc.so" not in ROOT.gSystem.GetLibraries(): 
    ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/NanoProc/python/plotter/functions.cc+" % os.environ['CMSSW_BASE']);
...
class TreeToYield:
    ...
    def _listFriendTrees(self):
        ...
        if 'FriendsSimple' in self._settings: friendSimpleOpts += [x for x in self._settings['FriendsSimple'].split(',')]
        if self._isNano:
...
#            print cut
    ...
    def getPlot(self,plotspec,cut,fsplit=None,closeTreeAfter=False,noUncertainties=False):
        if self._isVariation == None and noUncertainties == False:
            ...
            for (var,up,down) in variations.values():
    ...
    def getPlotRaw(self,name,expr,bins,cut,plotspec,fsplit=None,closeTreeAfter=False):
        ...
        if canKeys and histo.GetEntries() > 0 and histo.GetEntries() < self.getOption('KeysPdfMinN',2000) and not self._isdata and self.getOption("KeysPdf",False):
            ...
            if "/TH1Keys_cc.so" not in ROOT.gSystem.GetLibraries(): 
                ROOT.gROOT.ProcessLine(".L %s/src/CMGTools/NanoProc/python/plotter/TH1Keys.cc+" % os.environ['CMSSW_BASE']);
            (nb,xmin,xmax) = bins.split(",")
...
def addTreeToYieldOptions(parser):
    parser.add_option("-l", "--lumi",           dest="lumi",   type="string", default=None, help="Luminosity (in 1/fb)");
